 ...
 * 各场馆客流统计 Service 接口
 * <p>
 * 业务逻辑：遍历 table_venue_info 中的每个场馆，调用 HTTP API
...
public interface IVenueFlowService {
    ...
    /**
     * 同步所有场馆的客流数据到数据库。
     * <p>遍历所有场馆调用 HTTP API（今日进场/当前在场/峰值/平均停留），按 (dataDate, venueId) 唯一键写库。
     * 单个场馆同步失败不影响其他场馆（容错）。</p>
     *
     * @return 本次成功同步的场馆数
     */
    int syncAllVenueFlowFromApi();
     ...
     * @return 是否同步成功
     */
    boolean syncOneVenueFlowFromApi(Long venueId);
     ...
     * @return 各场馆客流 VO 列表
     */
    List<VenueFlowVO> queryByDate(LocalDate date);
}