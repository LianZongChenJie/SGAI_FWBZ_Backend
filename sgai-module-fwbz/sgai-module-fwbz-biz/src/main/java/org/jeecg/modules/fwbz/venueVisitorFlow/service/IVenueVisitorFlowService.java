 ...
 * 场馆客流统计 Service 接口
 * <p>
 * 业务逻辑：调用海康四个 OpenAPI（今日总客流/当前在场/峰值客流/平均停留）
...
public interface IVenueVisitorFlowService {
    ...
    /**
     * 同步海康四项客流数据到数据库（今日记录存在则更新，不存在则新增）。
     * 四个 API 任意一个失败不会影响其他项的同步（容错）。
     * <p>由定时任务 VenueVisitorFlowJob 每5分钟自动调用，也可通过 /syncFromHikvision 接口手动触发。</p>
     *
     * @return 本次同步成功的项数
     */
